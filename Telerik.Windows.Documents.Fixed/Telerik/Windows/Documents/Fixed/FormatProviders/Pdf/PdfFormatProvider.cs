	...
	public class PdfFormatProvider : BinaryFormatProviderBase<RadFixedDocument>
	{
		...
		void RecalculateMissingWidgetAppearances()
		{
			...
			FormField[] array = (from field in this.document.PublicDocument.AcroForm.FormFields
				where field.Widgets.Any((Widget widget) => PdfFormatProvider.ShouldRecalculateWidgetAppearance(widget, shouldRecalculateAllAppearances))
				select field).ToArray<FormField>();
			if (array.Length > 0)
			{
				foreach (FormField formField in array)
				{
					using (IEnumerator<Widget> enumerator = (from widget in formField.Widgets
						where PdfFormatProvider.ShouldRecalculateWidgetAppearance(widget, shouldRecalculateAllAppearances)
						select widget).GetEnumerator())
					{
