...
namespace Telerik.Windows.Documents.Fixed.FormatProviders.Pdf.Model.Elements.Fonts
{
	abstract class CidFontObject : PdfObject
	{
		...
		internal virtual void CopyPropertiesToFont(CidFontBase font, PostScriptReader reader, IPdfImportContext context)
		{
			this.FontDescriptor.Lang.CopyToProperty(font.Lang, (PdfName language) => language.Value);
			this.FontDescriptor.CidSet.CopyToProperty(font.CidSet, (CidSet cidSet) => cidSet.Data);
			this.CidSystemInfo.CopyToProperty(font.CidSystemInfo, (CidSystemInfo info) => new CidSystemInfo(info.Registry.ToString(), info.Ordering.ToString(), info.Supplement.Value));
			this.Widths.CopyToProperty(font.Widths, (PdfArray widthsArray) => CidFontObject.CreateCidWidths<int>(widthsArray, 1, new Func<Queue<int>, int>(CidFontObject.GetConstantWidth), new Func<PdfArray, IEnumerable<int>>(CidFontObject.GetWidthsArrayRange), context));
			this.WidthsVertical.CopyToProperty(font.WidthsVertical, (PdfArray widthsArray) => CidFontObject.CreateCidWidths<CidWidthVertical>(widthsArray, 3, new Func<Queue<int>, CidWidthVertical>(CidFontObject.GetConstantWidthVertical), new Func<PdfArray, IEnumerable<CidWidthVertical>>(CidFontObject.GetWidthsVerticalArrayRange), context));
			this.DefaultWidth.CopyToProperty(font.DefaultWidth, (PdfInt width) => width.Value);
			this.DefaultWidthVertical.CopyToProperty(font.DefaultWidthsVertical, (PdfArray array) => CidFontObject.GetDefaultWidthVertical(array));
			this.CharIdToGlyphIdMapping.CopyToProperty(font.CharIdToGlyphIdMapping, (CharIdToGlyphIdMappingBase mapping) => CidFontObject.CreateCidToGidMap(mapping));
		}
