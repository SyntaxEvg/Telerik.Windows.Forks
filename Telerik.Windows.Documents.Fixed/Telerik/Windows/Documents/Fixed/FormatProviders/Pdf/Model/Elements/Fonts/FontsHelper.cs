...
namespace Telerik.Windows.Documents.Fixed.FormatProviders.Pdf.Model.Elements.Fonts
{
	class FontsHelper
	{
		...
		internal static string StripFontName(string fontName)
		{
			...
			if (num > 0)
			{
				return fontName.Substring(num + 1).Split(Characters.FontFamilyDelimiters)[0];
			}
			return fontName.Split(Characters.FontFamilyDelimiters)[0];
		}
		...
		static string GetFontStylesFromFontName(string fontName)
		{
			string[] array = fontName.Split(Characters.FontFamilyDelimiters);
			if (array.Length > 1)
			{
				return array[1];
			}
