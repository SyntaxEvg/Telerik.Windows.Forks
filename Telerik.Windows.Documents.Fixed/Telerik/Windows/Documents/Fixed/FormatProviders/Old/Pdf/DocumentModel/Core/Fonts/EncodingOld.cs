...
namespace Telerik.Windows.Documents.Fixed.FormatProviders.Old.Pdf.DocumentModel.Core.Fonts
{
	class EncodingOld : PdfObjectOld, ISimpleEncoding, IEncoding
	{
		...
		public IEnumerable<Tuple<char, CharCodeOld>> Encode(byte[] bytes)
		{
			if (bytes != null)
			{
				this.Initialize();
				...
				{
					yield return new Tuple<char, CharCodeOld>(AdobeGlyphList.GetUnicode(this.names[(int)b]), new CharCodeOld(b));
				}
			}
			yield break;
		}
