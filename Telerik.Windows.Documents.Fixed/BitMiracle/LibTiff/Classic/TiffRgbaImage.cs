	...
	class TiffRgbaImage
	{
			...
			while (height-- > 0)
			{
				...
				for (i = width; i >= 8; i -= 8)
				{
					for (int j = 0; j < 8; j++)
					{
						...
						short r = num2 * (short)(byte.MaxValue - buffer[offset]) / 255;
						short g = num2 * (short)(byte.MaxValue - buffer[offset + 1]) / 255;
						short b = num2 * (short)(byte.MaxValue - buffer[offset + 2]) / 255;
				...
				if (i > 0 && i <= 7 && i > 0)
				{
					for (int k = i; k > 0; k--)
					{
						...
						short r2 = num3 * (short)(byte.MaxValue - buffer[offset]) / 255;
						short g2 = num3 * (short)(byte.MaxValue - buffer[offset + 1]) / 255;
						short b2 = num3 * (short)(byte.MaxValue - buffer[offset + 2]) / 255;
		...
		static void putRGBcontig8bitCMYKMaptile(TiffRgbaImage img, int[] raster, int rasterOffset, int rasterShift, int x, int y, int width, int height, byte[] buffer, int offset, int bufferShift)
		{
			...
			while (height-- > 0)
			{
				...
				{
					short num2 = (short)(byte.MaxValue - buffer[offset + 3]);
					short num3 = num2 * (short)(byte.MaxValue - buffer[offset]) / 255;
					short num4 = num2 * (short)(byte.MaxValue - buffer[offset + 1]) / 255;
					short num5 = num2 * (short)(byte.MaxValue - buffer[offset + 2]) / 255;
