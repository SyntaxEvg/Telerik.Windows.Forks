...
namespace Telerik.Windows.Documents.Spreadsheet.Core.DataStructures
{
	class CompressedList<TValue> : ICompressedList<TValue>, ICompressedList, ITranslateableCollection, IEnumerable<Range<long, TValue>>, IEnumerable
	{
		...
		ICompressedList<TValue> ICompressedList<T>.GetValue(long fromIndex, long toIndex)
		{
		...
		{
			return ((IEnumerable<Range<long, !0>>)this).GetEnumerator();
		}
		...
		void ICompressedList<T>.Sort(long fromIndex, long toIndex, int[] sortedIndexes, Func<long, ValueBox<TValue>, ValueBox<TValue>> translateValue)
		{
