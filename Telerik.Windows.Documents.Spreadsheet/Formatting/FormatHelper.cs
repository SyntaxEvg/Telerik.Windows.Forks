...
namespace Telerik.Windows.Documents.Spreadsheet.Formatting
{
	public static class FormatHelper
	{
		...
		internal static string ExpandSymbol(string symbol, double availableWidth, FontProperties fontProperties)
		{
			TextMeasurementInfo textMeasurementInfo = RadTextMeasurer.Measure(symbol, fontProperties, null);
			double width = textMeasurementInfo.Size.Width;
			int count = (int)(availableWidth / width);
			string text = string.Concat(Enumerable.Repeat<string>(symbol, count));
			double width2 = RadTextMeasurer.Measure(text, fontProperties, null).Size.Width;
			while (width2 - availableWidth > 0.1 && text.Length > 0)
			{
				text = text.Substring(0, text.Length - 1);
				width2 = RadTextMeasurer.Measure(text, fontProperties, null).Size.Width;
			}
			return text;
		}
