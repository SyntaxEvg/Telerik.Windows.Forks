...
namespace Telerik.Windows.Documents.Spreadsheet.FormatProviders.OpenXml.Xlsx.Contexts
{
	class XlsxWorksheetExportContext : XlsxWorksheetContextBase, IXlsxWorksheetExportContext, IOpenXmlExportContext
	{
		...
		public IEnumerable<CellInfo> GetNonEmptyCellsInRow(int rowIndex)
		{
			ICompressedList<bool> usedRangesInRow = new CompressedList<bool>(0L, (long)(SpreadsheetDefaultValues.ColumnCount - 1));
			foreach (CellPropertyDataInfo cellPropertyDataInfo in this.rowPropertyDataInfosToRespect)
			{
				Range rowUsedRange = cellPropertyDataInfo.GetRowUsedRange(rowIndex);
				if (rowUsedRange != null)
				{
					usedRangesInRow.SetValue((long)rowUsedRange.Start, (long)rowUsedRange.End, true);
				}
			}
			ICompressedList<bool> nonDefaultFormattingCompressedList = this.NonDefaultFormattingCompressedList[base.Worksheet.Cells];
			ICompressedList<ICellValue> valuesCompressedList = base.Worksheet.Cells.PropertyBag.GetPropertyValueCollection<ICellValue>(CellPropertyDefinitions.ValueProperty);
			foreach (Range<long, bool> range in usedRangesInRow.GetNonDefaultRanges())
			{
				...
				while ((long)columnIndex <= range.End)
				{
					...
					{
						yield return new CellInfo(rowIndex, columnIndex, base.Worksheet.Cells);
					}
					columnIndex++;
				}
			}
