...
{
	public class CoupPcd : NumbersInFunction
	{
		...
		static CoupPcd()
		{
			string description = "Returns a number that represents the previous coupon date before the settlement date.";
			string descriptionLocalizationKey = "Spreadsheet_Functions_CoupPcd_Info";
			...
			CoupPcd.Info = new FunctionInfo(CoupPcd.FunctionName, FunctionCategory.Financial, description, requiredArgumentsInfos, optionalArgumentsInfos, 1, false, descriptionLocalizationKey);
		}
		...
		protected override RadExpression EvaluateOverride(FunctionEvaluationContext<double> context)
		{
			...
			try
			{
				DateTime dateTime3 = FinancialFunctions.COUPPCD(dateTime.Value, dateTime2.Value, num, num2);
				number = FormatHelper.ConvertDateTimeToDouble(dateTime3);
			}
			catch (Exception)
			{
				return ErrorExpressions.NumberError;
		...
		public static readonly string FunctionName = "COUPPCD";
