...
namespace Telerik.Windows.Documents.Spreadsheet.Formatting.FormatStrings
{
	...
	{
		private List<FormatDescriptor> ConditionalFormatDescriptors { get; set; }
		...
		private void HandleStar(string sbPart, int index, out int newIndex, FormatDescriptor formatDescriptor)
		{
			newIndex = index;
		...
		private void HandleQuestionMarkSymbol(string sbPart, int index, out int newIndex, FormatDescriptor formatDescriptor)
		{
			NumberFormatStringParser._c__DisplayClass1 CS_8__locals1 = new NumberFormatStringParser._c__DisplayClass1();
			CS_8__locals1.sbPart = sbPart;
			CS_8__locals1._4__this = this;
			base.GetConsecutiveSymbolRange(CS_8__locals1.sbPart, index, out newIndex);
			int num = newIndex - index + 1;
			CS_8__locals1.nominatorRequiredLength = num;
			if (newIndex + 1 >= CS_8__locals1.sbPart.Length || CS_8__locals1.sbPart[newIndex + 1] != '/')
			{
				base.AddCurrentSequenceToItemsAndClear(formatDescriptor, true);
				string text = string.Concat<char>(Enumerable.Repeat<char>(CS_8__locals1.sbPart[index], num));
			...
			bool isNextSymbolQuestionMark = CS_8__locals1.sbPart[newStartIndex] == '?';
			if (isNextSymbolQuestionMark)
			{
				base.GetConsecutiveSymbolRange(CS_8__locals1.sbPart, newStartIndex, out newIndex);
			}
			else
			{
				this.GetConsecutiveDigitRange(CS_8__locals1.sbPart, newStartIndex, out newIndex);
			}
			...
			Func<double?, string, IEnumerable<FormatDescriptorItem>> getItems = delegate(double? doubleValue, string startSequence)
			{
				...
				if (isNextSymbolQuestionMark)
				{
					...
					upToDigitsString = CS_8__locals1._4__this.GetUpToDigits(Math.Abs(num3), num4);
				}
				else
				{
					denominatorRequiredLength = newIndexCopy - newStartIndex + 1;
					string s = CS_8__locals1.sbPart.Substring(newStartIndex, denominatorRequiredLength);
					upToDigitsString = CS_8__locals1._4__this.GetFractionStringByDenominator(Math.Abs(num3), int.Parse(s), true);
				}
				if (upToDigitsString.IndexOf('/') == -1)
				{
					...
					string str = string.Concat<char>(Enumerable.Repeat<char>(FormatHelper.FractionsInvisibleChar, CS_8__locals1.nominatorRequiredLength));
				...
				else
				{
					...
					{
						CS_8__locals1._4__this.valueToAdd = -1;
					}
					else if (num3 <= -0.5)
					{
						CS_8__locals1._4__this.valueToAdd = 1;
					...
					if (text2.Length < CS_8__locals1.nominatorRequiredLength || text3.Length < denominatorRequiredLength)
					{
						if (!string.IsNullOrEmpty(startSequence))
						{
							list.Add(new FormatDescriptorItem(startSequence, false, false, true));
						...
						if (text2.Length < CS_8__locals1.nominatorRequiredLength)
						{
							string text4 = string.Concat<char>(Enumerable.Repeat<char>(FormatHelper.FractionsInvisibleChar, CS_8__locals1.nominatorRequiredLength - text2.Length));
		...
		private string GetFractionStringByDenominator(double value, int denom, bool isResultStringEmpty)
		{
		...
		private string GetUpToDigits(double number, int upTo)
		{
		...
		private List<Tuple<int, int>> GetApproximateFractions(double number)
		{
		...
		private ulong GetMaxNumerator(double f)
		{
		...
		private bool TryParseSymbolCulture(string modificator, FormatDescriptor formatDescriptor)
		{
		...
		private bool TryParseCondition(string modificator, FormatDescriptor formatDescriptor)
		{
		...
		private void EscapeItemsText(FormatDescriptor formatDescriptor)
		{
			...
			foreach (FormatDescriptorItem formatDescriptorItem in formatDescriptor.Items.Where((FormatDescriptorItem fpi) => fpi.ApplyFormat))
			{
		...
		private string GetFormatForScientificNumber(double number)
		{
		...
		private void ValidateNumberFormatDescriptors(List<FormatDescriptor> formatStringParts)
		{
		...
		[CompilerGenerated]
		private static bool EscapeItemsTextb__11(FormatDescriptorItem fpi)
		{
			return fpi.ApplyFormat;
		}

		private int valueToAdd;

		[CompilerGenerated]
		private List<FormatDescriptor> ConditionalFormatDescriptorsk__BackingField;

		[CompilerGenerated]
		private static Func<FormatDescriptorItem, bool> CS_9__CachedAnonymousMethodDelegate12;

		[CompilerGenerated]
		private sealed class _c__DisplayClass1
		{
			public _c__DisplayClass1()
			{
			}

			public int nominatorRequiredLength;

			public NumberFormatStringParser _4__this;

			public string sbPart;
		}

		[CompilerGenerated]
		private sealed class _c__DisplayClass3
		{
			public _c__DisplayClass3()
			{
			}

			public IEnumerable<FormatDescriptorItem> HandleQuestionMarkSymbolb__0(double? doubleValue, string startSequence)
			{
				List<FormatDescriptorItem> list = new List<FormatDescriptorItem>();
				double num = FormatHelper.Round(doubleValue.Value);
				double num2 = doubleValue.Value - num;
				if (this.isNextSymbolQuestionMark)
				{
					int upTo = this.newIndexCopy - this.newStartIndex + 1;
					this.denominatorRequiredLength = upTo;
					this.upToDigitsString = this.CS_8__locals2._4__this.GetUpToDigits(Math.Abs(num2), upTo);
				}
				else
				{
					this.denominatorRequiredLength = this.newIndexCopy - this.newStartIndex + 1;
					string s = this.CS_8__locals2.sbPart.Substring(this.newStartIndex, this.denominatorRequiredLength);
					this.upToDigitsString = this.CS_8__locals2._4__this.GetFractionStringByDenominator(Math.Abs(num2), int.Parse(s), true);
				}
				if (this.upToDigitsString.IndexOf('/') == -1)
				{
					if (this.upToDigitsString == "0" && num == 0.0)
					{
						startSequence = startSequence.Replace('#', '0');
					}
					if (!string.IsNullOrEmpty(startSequence))
					...
					string str = string.Concat<char>(Enumerable.Repeat<char>(FormatHelper.FractionsInvisibleChar, this.CS_8__locals2.nominatorRequiredLength));
					string str2 = string.Concat<char>(Enumerable.Repeat<char>(FormatHelper.FractionsInvisibleChar, this.denominatorRequiredLength));
					list.Add(new FormatDescriptorItem(str + "/" + str2, true, false, false));
				}
				else
				{
					string text = this.upToDigitsString;
					int num3 = text.IndexOf('/');
					string text2 = text.Substring(0, num3);
					string text3 = text.Substring(num3 + 1);
					if (num2 >= 0.5 && num > 0.0)
					{
						this.CS_8__locals2._4__this.valueToAdd = -1;
					}
					else if (num2 <= -0.5)
					{
						this.CS_8__locals2._4__this.valueToAdd = 1;
					...
					if (text2.Length < this.CS_8__locals2.nominatorRequiredLength || text3.Length < this.denominatorRequiredLength)
					{
						if (!string.IsNullOrEmpty(startSequence))
						{
							list.Add(new FormatDescriptorItem(startSequence, false, false, true));
						...
						if (text2.Length < this.CS_8__locals2.nominatorRequiredLength)
						{
							string text4 = string.Concat<char>(Enumerable.Repeat<char>(FormatHelper.FractionsInvisibleChar, this.CS_8__locals2.nominatorRequiredLength - text2.Length));
							list.Add(new FormatDescriptorItem(text4, true, false, false));
						}
						list.Add(new FormatDescriptorItem(text, false, false, false));
						if (text3.Length < this.denominatorRequiredLength)
						{
							string text5 = string.Concat<char>(Enumerable.Repeat<char>(FormatHelper.FractionsInvisibleChar, this.denominatorRequiredLength - text3.Length));
							list.Add(new FormatDescriptorItem(text5, true, false, false));
						}
					}
					else
			...
			public NumberFormatStringParser._c__DisplayClass1 CS_8__locals2;

			public int newStartIndex;

			public string upToDigitsString;

			public int denominatorRequiredLength;

			public bool isNextSymbolQuestionMark;

			public int newIndexCopy;
		}

		[CompilerGenerated]
		private sealed class _c__DisplayClassf
		{
			public _c__DisplayClassf()
			{
			}

			public bool TryParseConditionb__5(double num)
			{
				return (decimal)num <= this.number;
			}

			public bool TryParseConditionb__6(double num)
			{
				return (decimal)num >= this.number;
			}

			public bool TryParseConditionb__7(double num)
			{
				return (decimal)num > this.number;
			}

			public bool TryParseConditionb__8(double num)
			{
				return (decimal)num < this.number;
			}

			public bool TryParseConditionb__9(double num)
			{
				return (decimal)num == this.number;
			}
