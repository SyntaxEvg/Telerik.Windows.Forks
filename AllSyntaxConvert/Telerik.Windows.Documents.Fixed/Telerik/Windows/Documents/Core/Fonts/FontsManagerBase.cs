...
namespace Telerik.Windows.Documents.Core.Fonts
{
	abstract class FontsManagerBase
	{
		...
		{
			Guard.ThrowExceptionIfNullOrEmpty(fontName, "fontName");
			//string resourceName = string.Format("Syntax.Windows.Documents.Fixed.Resources.Fonts.{0}.pfb", fontName);
			string resourceName = string.Format("AllSyntaxConvert.Resours.Syntax.Windows.Documents.Fixed.Resources.Fonts.{0}.pfb", fontName);
		...
		{
			if (FontsManagerBase.standardFontStreams == null || FontsManagerBase.standardFontStreams.Count == 0)
			{
		...
		static  Dictionary<string, string> alternativeNames =new Dictionary<string, string>();

		static  Dictionary<string, StandardFontDescriptor> standardFontDescriptors = new Dictionary<string, StandardFontDescriptor>();

		static Dictionary<string, byte[]> standardFontStreams =new Dictionary<string, byte[]>();

	 Dictionary<string, Type1FontSource> standardFontSources =new Dictionary<string, Type1FontSource>();
	}
}
