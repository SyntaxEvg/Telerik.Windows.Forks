...
{
	abstract class Asn1Set : Asn1Object, IEnumerable
	{
		...
		class Asn1SetParserImpl : Asn1SetParser, IAsn1Convertible
		{
		...
		class DerComparer : IComparer
		{
			...
			bool AllZeroesFrom(byte[] bs, int pos)
			{
				while (pos < bs.Length)
				{
					if (bs[pos++] != 0)
